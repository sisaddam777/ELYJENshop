...
export interface EffectOptions {
  ...
  distortion: string | DistortionOptions;
  length: number;
  roadWidth: number;
  islandWidth: number;
  lanesPerRoad: number;
...
const Hyperspeed: React.FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  const appRef = useRef<any>(null); // Will type App class later

  const stableOptionsKey = useMemo(() => JSON.stringify(effectOptions), [effectOptions]);
  ...
  useEffect(() => {
    ...
    const mountainUniforms: ShaderUniforms = {
      uFreq: { value: new THREE.Vector3(3, 6, 10) },
      uAmp: { value: new THREE.Vector3(30, 30, 20) }
    };

    const xyUniforms: ShaderUniforms = {
      uFreq: { value: new THREE.Vector2(5, 2) },
      uAmp: { value: new THREE.Vector2(25, 15) }
    };

    const LongRaceUniforms: ShaderUniforms = {
      uFreq: { value: new THREE.Vector2(2, 3) },
      uAmp: { value: new THREE.Vector2(35, 10) }
    };

    const turbulentUniforms: ShaderUniforms = {
      uFreq: { value: new THREE.Vector4(4, 8, 8, 1) },
      uAmp: { value: new THREE.Vector4(25, 5, 10, 10) }
    };

    const deepUniforms: ShaderUniforms = {
    ...
    const distortions: { [key: string]: DistortionOptions } = {
      mountainDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const getX = (p: number) => Math.cos(p * Math.PI * uFreq.x + time) * uAmp.x;
          const getY = (p: number) => nsin(p * Math.PI * uFreq.y + time) * uAmp.y;
          const getZ = (p: number) => nsin(p * Math.PI * uFreq.z + time) * uAmp.z;

          let distortion = new THREE.Vector3(
            getX(progress) - getX(movementProgressFix),
            getY(progress) - getY(movementProgressFix),
            getZ(progress) - getZ(movementProgressFix)
          );
      ...
      xyDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const getX = (p: number) => Math.cos(p * Math.PI * uFreq.x + time) * uAmp.x;
          const getY = (p: number) => Math.sin(p * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y;
      ...
      LongRaceDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const getY = (p: number) => Math.sin(p * Math.PI * uFreq.y + time) * uAmp.y;
    ...
    class App {
      ...
      scene: THREE.Scene;
      fogUniforms: { [key: string]: THREE.IUniform };
      timer: THREE.Timer;
      assets: any;
      disposed: boolean;
      road: Road;
      ...
      constructor(container: HTMLElement, options: EffectOptions) {
        ...
        if (this.options.distortion == null || typeof this.options.distortion === 'string') {
      ...
      update(delta: number) {
        ...
        const distortionOptions = this.options.distortion as DistortionOptions;
        if (distortionOptions.getJS) {
          const distortion = distortionOptions.getJS(0.025, time);
      ...
      init() {
        ...
        let material = new THREE.ShaderMaterial({
          ...
          uniforms: Object.assign(
            ...
            (options.distortion as DistortionOptions).uniforms
          )
        });
        ...
        material.onBeforeCompile = shader => {
          shader.vertexShader = shader.vertexShader.replace(
            ...
            (options.distortion as DistortionOptions).getDistortion
          );
        };
    ...
    class LightsSticks {
      ...
      init() {
        ...
        if (Array.isArray(options.colors.sticks)) {
          ...
          colors = new THREE.Color(options.colors.sticks);
        }
    ...
    class Road {
      ...
      createPlane(side: number, width: number, isRoad: boolean) {
        ...
        const material = new THREE.ShaderMaterial({
          ...
          side: THREE.DoubleSide,
          uniforms: Object.assign(uniforms, this.webgl.fogUniforms, (options.distortion as DistortionOptions).uniforms)
        });
    ...
    // Validate and fall back for distortion
    if (typeof options.distortion === 'string') {
      const distortionKey = options.distortion;
      if (distortions[distortionKey as keyof typeof distortions]) {
        options.distortion = distortions[distortionKey as keyof typeof distortions];
      } else {
        console.warn(`Hyperspeed: Distortion '${distortionKey}' not found. Falling back to default.`);
        const fallbackKey = DEFAULT_EFFECT_OPTIONS.distortion as keyof typeof distortions;
        options.distortion = distortions[fallbackKey] || {
          uniforms: distortion_uniforms,
          getDistortion: distortion_vertex
        };
      }
    ...
    // Final sanity check to ensure distortion object has required properties
    if (
      !options.distortion ||
      ...
      console.error('Hyperspeed: Failed to obtain valid distortion object. Using hard fallback.');
      options.distortion = {
        uniforms: distortion_uniforms,
        getDistortion: distortion_vertex
      };
